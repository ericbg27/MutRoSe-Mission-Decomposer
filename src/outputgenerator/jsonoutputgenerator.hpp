#ifndef __JSONOUTPUTGENERATOR
#define __JSONOUTPUTGENERATOR
...
class JSONOutputGenerator : public FileOutputGenerator {
    public:
        void generate_instances_output(std::vector<SemanticMapping> semantic_mapping, std::map<std::string,set<std::string>> sorts, std::vector<sort_definition> sort_definitions, std::vector<predicate_definition> predicate_definitions,
                                            std::map<std::string, std::variant<std::pair<std::string,std::string>,std::pair<std::vector<std::string>,std::string>>> gm_var_map, std::set<std::string> robot_related_sorts);
