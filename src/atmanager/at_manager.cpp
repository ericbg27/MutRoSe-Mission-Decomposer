...
void FileKnowledgeATManager::recursive_at_instances_generation(int current, int depth, map<int,int>& node_depths, pt::ptree world_tree, vector<VariableMapping> var_mapping,
	...
	if(gm[current].type == istar_goal) {
		...
		if(goal_type == query_goal_type) {
			pt::ptree queried_tree = get_query_ptree(gm, current, valid_variables, valid_forAll_conditions, world_tree);
			QueriedProperty q = std::get<QueriedProperty>(gm[current].custom_props[queried_property_prop]);

			solve_query_statement(queried_tree, q, gm, current, valid_variables, gm_var_map);
		} else if(goal_type == achieve_goal_type) {
			AchieveCondition a = std::get<AchieveCondition>(gm[current].custom_props[achieve_condition_prop]);
			if(a.has_forAll_expr) {
				valid_forAll_conditions[depth] = a;
				forAll_inst_id[depth] = 1;

				vector<int> children = gm[current].children;
				
				string iterated_var_type;
				vector<pair<string,string>> controlled_vars = std::get<vector<pair<string,string>>>(gm[current].custom_props[controls_prop]);
		...
		*/
		...
		if(gm[current].custom_props.find(location_prop) != gm[current].custom_props.end()) {
			string location_var;
			location_var = std::get<string>(gm[current].custom_props[location_prop]);
		...
		if(gm[current].custom_props.find(params_prop) != gm[current].custom_props.end()) {
			vector<string> params = std::get<vector<string>>(gm[current].custom_props["Params"]);
