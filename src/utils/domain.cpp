...
pair<task,bool> flatten_primitive_mdp_task(parsed_task & a) {
	// expand effects and preconditions (if necessary and possible)
	vector<pair<pair<vector<variant<literal,conditional_effect,reward_change,conditional_reward_change>>,vector<literal> >, additional_variables> > elist;
	vector<pair<pair<vector<variant<literal,conditional_effect,reward_change,conditional_reward_change>>, vector<literal> >, additional_variables> > plist; 

	elist = a.eff->mdp_expand();
	plist = a.prec->mdp_expand(); // precondition cannot contain conditional effects
	...
	for(auto e : elist) for(auto p : plist){
		...
		for(variant<literal,conditional_effect, reward_change, conditional_reward_change> pl : p.first.first){
		...
		for (auto eff : e.first.first){
			if (holds_alternative<literal>(eff)){
				...
				if(holds_alternative<reward_change>(eff)) {
					t.rwd.push_back(get<reward_change>(eff));
				} else {
...
void parsed_method_to_data_structures(bool compileConditionalEffects,
	...
	for (auto e : parsed_methods) for (parsed_method pm : e.second) {
		...
		mPrec_task.required_capabilities = new vector<string>();
		...
		/*cout << "Method precondition task named " << mPrec_task.name << " args: " << endl;
		for(auto arg : mPrec_task.arguments->vars) {
...
conditional_reward_change::conditional_reward_change(vector<literal> cond, reward_change r) {
	condition = cond;
	rwd = r;
}
