...
vector<pair<pair<vector<variant<literal,conditional_effect>>,vector<literal> >, additional_variables> > general_formula::expand(bool compileConditionalEffects){
	...
	if (this->type == WHEN) {
		...
		//vector<pair<pair<vector<variant<literal,conditional_effect>>,vector<literal> >, additional_variables> >
		if (compileConditionalEffects){
...
reward_change general_formula::rewardLiteral() {
	assert(this->type == REWARD);

	reward_change r;
	r.increase = true;
	r.isRewardChangeExpression = false;
	r.name = this->predicate;
	r.type = NONE;
...
parsed_task copy_parsed_task(parsed_task& source) {
	...
	var_declaration* as = new var_declaration();
	for(unsigned int i = 0;i < source.arguments->vars.size();i++) {
		pair<string,string> aux;
		aux.first = source.arguments->vars[i].first;
		aux.second = source.arguments->vars[i].second;
		as->vars.push_back(aux);
	}
	copy.arguments = as;
	map<string,string> empty_map;
	copy.eff = source.eff->copyReplace(empty_map);
	copy.prec = source.prec->copyReplace(empty_map);

	return copy;
}

vector<pair<pair<vector<variant<literal,conditional_effect,reward_change,conditional_reward_change>>,vector<literal> >, additional_variables> > general_formula::mdp_expand(){
	vector<pair<pair<vector<variant<literal,conditional_effect,reward_change,conditional_reward_change>>,vector<literal> >, additional_variables> > ret;

	//EMPTY Formula case
	if (this->type == EMPTY || (this->subformulae.size() == 0 &&
				(this->type == AND || this->type == OR || this->type == FORALL || this->type == EXISTS))){
		vector<variant<literal,conditional_effect,reward_change,conditional_reward_change>> empty1;
		vector<literal> empty2;
		additional_variables none;
		ret.push_back(make_pair(make_pair(empty1,empty2),none));
		return ret;
	...
	if (this->type == EXISTS){
		...
		vector<pair<pair<vector<variant<literal,conditional_effect,reward_change,conditional_reward_change>>, vector<literal> >, additional_variables> > cur = 
			this->subformulae[0]->mdp_expand();
		for(pair<string,string> var : this->qvariables.vars){
			vector<pair<pair<vector<variant<literal,conditional_effect,reward_change,conditional_reward_change>>, vector<literal> >, additional_variables> > prev = cur;
			cur.clear();
			string newName = var.first + "_" + to_string(++global_exists_variable_counter);
			
			for(auto old : prev){
				pair<pair<vector<variant<literal,conditional_effect,reward_change,conditional_reward_change>>,vector<literal> >,  additional_variables>	combined = old;
				combined.second.insert(make_pair(var_replace[var.first],var.second));
				cur.push_back(combined);
			}
		}
	...
	vector<vector<pair<pair<vector<variant<literal,conditional_effect,reward_change,conditional_reward_change>>, vector<literal> >, additional_variables> > > subresults;
	for(auto sub : this->subformulae) subresults.push_back(sub->mdp_expand());
	...
	//
	if (this->type == AND){
		vector<pair<pair<vector<variant<literal,conditional_effect,reward_change,conditional_reward_change>>,vector<literal> >, additional_variables> > cur = subresults[0];
		for(unsigned int i = 1; i < subresults.size(); i++){
			vector<pair<pair<vector<variant<literal,conditional_effect,reward_change,conditional_reward_change>>, vector<literal> >, additional_variables> > prev = cur;
			cur.clear();
			for(auto next : subresults[i]) for(auto old : prev){
				pair<pair<vector<variant<literal,conditional_effect,reward_change,conditional_reward_change>>, vector<literal> >, additional_variables> combined = old;
				for(variant<literal,conditional_effect,reward_change,conditional_reward_change> l : next.first.first) combined.first.first.push_back(l);
				for(literal l : next.first.second) combined.first.second.push_back(l);
				for(auto v : next.second) combined.second.insert(v);
				cur.push_back(combined);
			}
	...
	if (this->type == INTVALUE){
		...
		r.value = this->value;
		r.increase = true;
		r.fvalue = this->fvalue;
		r.predvalue = *(this->predvalue);
		r.type = INTEGER;
		r.isRewardChangeExpression = true;
		ls.push_back(r);
	...
	if (this->type == REWARD_CHANGE) {
		assert(subresults.size() == 2);
		assert(subresults[0].size() == 1);
		assert(subresults[0][0].first.first.size() == 1);
		assert(holds_alternative<literal>(subresults[0][0].first.first[0]));
		assert(std::count(rewards_definitions.begin(),rewards_definitions.end(),get<literal>(subresults[0][0].first.first[0]).predicate) > 0);
		//assert(get<reward_change>(subresults[0][0].first.first[0]).arguments.size() == 0);

		assert(subresults[1].size() == 1);
		assert(subresults[1][0].first.first.size() == 1);
		assert(holds_alternative<reward_change>(subresults[1][0].first.first[0]));
		get<reward_change>(subresults[1][0].first.first[0]).name = get<literal>(subresults[0][0].first.first[0]).predicate;
		ret.push_back(subresults[1][0]);
	}
	...
	if (this->type == WHEN) {
		...
		// effect might have multiple expansions (i.e. be disjunctive) we here assume that this means angelic non-determinism
		for (auto expanded_condition : subresults[0]) for (auto expanded_effect : subresults[1]){
			vector<variant<literal,conditional_effect,reward_change,conditional_reward_change>> eff = expanded_effect.first.first;
			vector<literal> cond = expanded_effect.first.second;
			
			for (variant<literal,conditional_effect,reward_change,conditional_reward_change> x : expanded_condition.first.first)
				if (holds_alternative<literal>(x)){
					cond.push_back(get<literal>(x));
				} else assert(false); // conditional effect inside condition
			...
			// we have to build the conditional effects
			vector<variant<literal,conditional_effect,reward_change,conditional_reward_change>> newEff;
			for (variant<literal,conditional_effect,reward_change,conditional_reward_change> & e : eff) {
				/*
					TODO
						- Support rewards and conditional rewards
						- Find a way to verify if we have a reward or not in order to check for conditional rewards
						- Possibly done!
				*/
				...
				if (holds_alternative<literal>(e)){
					...
					if(holds_alternative<conditional_effect>(e)) {
						// nested conditional effect
						vector<literal> innercond = get<conditional_effect>(e).condition;
						literal innerE = get<conditional_effect>(e).effect;
						innercond.insert(innercond.end(), cond.begin(),cond.end());
						...
						if(holds_alternative<reward_change>(e)) {
							newEff.push_back(conditional_reward_change(cond,get<reward_change>(e)));
						} else {
							// nested conditional reward change
							vector<literal> innercond = get<conditional_reward_change>(e).condition;
							reward_change innerR = get<conditional_reward_change>(e).rwd;
							innercond.insert(innercond.end(), cond.begin(),cond.end());
								
							newEff.push_back(conditional_reward_change(innercond,innerR));
						}
					}
				}
			}
