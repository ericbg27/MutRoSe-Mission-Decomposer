...
class ValidMissionGenerator {
    public:
        ...
        void recursive_valid_mission_decomposition(std::string last_op, std::queue<std::pair<int,ATNode>>& mission_queue, std::vector<std::pair<int,ATNode>>& possible_conflicts, std::map<int,std::vector<std::variant<ground_literal,std::pair<ground_literal,int>>>>& effects_to_apply, int depth);
        void resolve_conflicts(std::vector<std::pair<int,ATNode>> possible_conflicts);
        void apply_effects_and_check_conditions(std::map<int,std::vector<std::variant<ground_literal,std::pair<ground_literal,int>>>>& effects_to_apply, int depth, std::pair<int,ATNode> current_node);
